...
SqlStore.prototype._defineRelationModel = function (relation, relationName, many) {
  ...
  var delimeter = self.sequelize.options.define.underscored ? '_' : DEFAULT_RELATION_DELIMITER
  var modelName = self.resourceConfig.resource + delimeter + relationName
  var modelProperties = {
    uid: {
...
SqlStore.prototype._fixObject = function (json) {
  ...
  Object.keys(json).forEach(function (attribute) {
    if (attribute.indexOf(self.resourceConfig.resource + delimeter) !== 0) return

    var fixedName = attribute.split(self.resourceConfig.resource + delimeter).pop()
...
SqlStore.prototype._clearAndSetRelationTables = function (theResource, partialResource, t, callback) {
  ...
  Object.keys(self.relations).forEach(function (relationName) {
    ...
    var delimeter = self.sequelize.options.define.underscored ? '_' : DEFAULT_RELATION_DELIMITER
    var keyName = self.resourceConfig.resource + delimeter + relationName
