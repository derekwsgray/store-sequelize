...
var DEFAULT_RELATION_DELIMITER = '-'
var UNDERSCORED_RELATION_DELIMITER = '_'
...
SqlStore.prototype._buildModels = function () {
  ...
  var extraModelOptions = {
    timestamps: false
  };

  if (self.config.dialect === 'mssql') {
    extraModelOptions.hasTrigger = true
  }
  self.baseModel = self.sequelize.define(self.resourceConfig.resource, modelAttributes, extraModelOptions)
...
SqlStore.prototype._defineRelationModel = function (relation, relationName, many) {
  ...
  var delimiter = self.sequelize.options.define.underscored ? UNDERSCORED_RELATION_DELIMITER : DEFAULT_RELATION_DELIMITER
  var modelName = self.resourceConfig.resource + delimiter + relationName
  var modelProperties = {
    uid: {
  ...
  var relatedModel = self.sequelize.define(modelName, modelProperties, _.defaults({
    timestamps: false,
    indexes: [ { fields: [ 'id' ] } ],
    underscored: self.sequelize.options.define.underscored,
    freezeTableName: self.sequelize.options.define.freezeTableName
  }, {
    underscored: false,
    freezeTableName: true
  }))
...
SqlStore.prototype._fixObject = function (json) {
  ...
  Object.keys(json).forEach(function (attribute) {
    if (attribute.indexOf(self.resourceConfig.resource + delimiter) !== 0) return

    var fixedName = attribute.split(self.resourceConfig.resource + delimiter).pop()
...
SqlStore.prototype._clearAndSetRelationTables = function (theResource, partialResource, t, callback) {
  ...
  Object.keys(self.relations).forEach(function (relationName) {
    ...
    var delimiter = self.sequelize.options.define.underscored ? UNDERSCORED_RELATION_DELIMITER : DEFAULT_RELATION_DELIMITER
    var keyName = self.resourceConfig.resource + delimiter + relationName
    ...
    if (newResource.id === 0) {
      ...
      if (!isNaN(_.toNumber(newResource.id))) {
        ...
        self.sequelize.query(`ALTER SEQUENCE "${newResource.type}_id_seq" RESTART WITH ${parseInt(newResource.id) + 1};`)
          .then(function (err5, result) {})
      }
    }
    self.baseModel.create(newResource, t).then(function (err2, theResource) {
      if (err2) return finishTransaction(err2)

      return self.sequelize.transaction((transaction) => {
        return self._clearAndSetRelationTables(theResource, newResource, transaction, function (err) {
...
SqlStore.prototype.delete = function (request, finishedCallback) {
  ...
  self._dealWithTransaction(finishedCallback, function (t, finishTransaction) {
    self.baseModel.findAll({
      ...
      theResource.destroy(t).then(function (deleteErr) {
